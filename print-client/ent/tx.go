...
func (tx *Tx) init() {
	tx.LocalDevice = NewLocalDeviceClient(tx.config)
	tx.LogError = NewLogErrorClient(tx.config)
	tx.PrintJob = NewPrintJobClient(tx.config)
	tx.RecentScan = NewRecentScanClient(tx.config)
	tx.RemoteConnection = NewRemoteConnectionClient(tx.config)
	tx.UniqueComputer = NewUniqueComputerClient(tx.config)
}
...
// applies a query, for example: LocalDevice.QueryXXX(), the query will be executed
