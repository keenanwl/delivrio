...
type PlanQuery struct {
	...
	withPlanHistoryPlan      *PlanHistoryQuery
	modifiers                []func(*sql.Selector)
	loadTotal                []func(context.Context, []*Plan) error
	withNamedTenant          map[string]*TenantQuery
	withNamedPlanHistoryPlan map[string]*PlanHistoryQuery
...
func (pq *PlanQuery) QueryTenant() *TenantQuery {
...
// QueryPlanHistoryPlan chains the current query on the "plan_history_plan" edge.
func (pq *PlanQuery) QueryPlanHistoryPlan() *PlanHistoryQuery {
	query := (&PlanHistoryClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PlanQuery) FirstID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (pq *PlanQuery) FirstIDX(ctx context.Context) pulid.ID {
...
func (pq *PlanQuery) OnlyID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (pq *PlanQuery) OnlyIDX(ctx context.Context) pulid.ID {
...
func (pq *PlanQuery) IDs(ctx context.Context) (ids []pulid.ID, err error) {
...
func (pq *PlanQuery) IDsX(ctx context.Context) []pulid.ID {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pq *PlanQuery) Clone() *PlanQuery {
	...
	return &PlanQuery{
		...
		withPlanHistoryPlan: pq.withPlanHistoryPlan.Clone(),
...
func (pq *PlanQuery) WithTenant(opts ...func(*TenantQuery)) *PlanQuery {
	query := (&TenantClient{config: pq.config}).Query()
...
// WithPlanHistoryPlan tells the query-builder to eager-load the nodes that are connected to
// the "plan_history_plan" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PlanQuery) WithPlanHistoryPlan(opts ...func(*PlanHistoryQuery)) *PlanQuery {
	query := (&PlanHistoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPlanHistoryPlan = query
	return pq
}
...
func (pq *PlanQuery) prepareQuery(ctx context.Context) error {
	...
	if err := plan.Policy.EvalQuery(ctx, pq); err != nil {
		return err
	}
	return nil
}
...
func (pq *PlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plan, error) {
	...
	if query := pq.withTenant; query != nil {
		if err := pq.loadTenant(ctx, query, nodes,
			func(n *Plan) { n.Edges.Tenant = []*Tenant{} },
			func(n *Plan, e *Tenant) { n.Edges.Tenant = append(n.Edges.Tenant, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withPlanHistoryPlan; query != nil {
		if err := pq.loadPlanHistoryPlan(ctx, query, nodes,
			func(n *Plan) { n.Edges.PlanHistoryPlan = []*PlanHistory{} },
			func(n *Plan, e *PlanHistory) { n.Edges.PlanHistoryPlan = append(n.Edges.PlanHistoryPlan, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedTenant {
		if err := pq.loadTenant(ctx, query, nodes,
			func(n *Plan) { n.appendNamedTenant(name) },
			func(n *Plan, e *Tenant) { n.appendNamedTenant(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedPlanHistoryPlan {
		if err := pq.loadPlanHistoryPlan(ctx, query, nodes,
			func(n *Plan) { n.appendNamedPlanHistoryPlan(name) },
			func(n *Plan, e *PlanHistory) { n.appendNamedPlanHistoryPlan(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PlanQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *Tenant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[pulid.ID]*Plan)
	for i := range nodes {
	...
	query.Where(predicate.Tenant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plan.TenantColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.plan_tenant
		if fk == nil {
			return fmt.Errorf(`foreign-key "plan_tenant" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "plan_tenant" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *PlanQuery) loadPlanHistoryPlan(ctx context.Context, query *PlanHistoryQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *PlanHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[pulid.ID]*Plan)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PlanHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plan.PlanHistoryPlanColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.plan_plan_history_plan
		if fk == nil {
			return fmt.Errorf(`foreign-key "plan_plan_history_plan" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "plan_plan_history_plan" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *PlanQuery) WithNamedTenant(name string, opts ...func(*TenantQuery)) *PlanQuery {
	query := (&TenantClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedTenant == nil {
		pq.withNamedTenant = make(map[string]*TenantQuery)
	}
	pq.withNamedTenant[name] = query
	return pq
}

// WithNamedPlanHistoryPlan tells the query-builder to eager-load the nodes that are connected to the "plan_history_plan"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *PlanQuery) WithNamedPlanHistoryPlan(name string, opts ...func(*PlanHistoryQuery)) *PlanQuery {
	query := (&PlanHistoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedPlanHistoryPlan == nil {
		pq.withNamedPlanHistoryPlan = make(map[string]*PlanHistoryQuery)
	}
	pq.withNamedPlanHistoryPlan[name] = query
	return pq
}
