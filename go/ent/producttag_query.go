...
type ProductTagQuery struct {
	...
	loadTotal         []func(context.Context, []*ProductTag) error
...
func (ptq *ProductTagQuery) QueryTenant() *TenantQuery {
	query := (&TenantClient{config: ptq.config}).Query()
...
func (ptq *ProductTagQuery) FirstID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (ptq *ProductTagQuery) FirstIDX(ctx context.Context) pulid.ID {
...
func (ptq *ProductTagQuery) OnlyID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (ptq *ProductTagQuery) OnlyIDX(ctx context.Context) pulid.ID {
...
func (ptq *ProductTagQuery) IDs(ctx context.Context) (ids []pulid.ID, err error) {
...
func (ptq *ProductTagQuery) IDsX(ctx context.Context) []pulid.ID {
	ids, err := ptq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (ptq *ProductTagQuery) Clone() *ProductTagQuery {
	...
	return &ProductTagQuery{
		...
		withTenant:   ptq.withTenant.Clone(),
...
func (ptq *ProductTagQuery) WithTenant(opts ...func(*TenantQuery)) *ProductTagQuery {
	query := (&TenantClient{config: ptq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ptq.withTenant = query
	return ptq
}
...
//		GroupBy(producttag.FieldTenantID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ProductTag.Query().
//		Select(producttag.FieldTenantID).
//		Scan(ctx, &v)
...
func (ptq *ProductTagQuery) prepareQuery(ctx context.Context) error {
	...
	if producttag.Policy == nil {
		return errors.New("ent: uninitialized producttag.Policy (forgotten import ent/runtime?)")
	}
	if err := producttag.Policy.EvalQuery(ctx, ptq); err != nil {
		return err
	}
	return nil
}
...
func (ptq *ProductTagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProductTag, error) {
	...
	if query := ptq.withTenant; query != nil {
		if err := ptq.loadTenant(ctx, query, nodes, nil,
			func(n *ProductTag, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range ptq.withNamedProducts {
		if err := ptq.loadProducts(ctx, query, nodes,
			func(n *ProductTag) { n.appendNamedProducts(name) },
			func(n *ProductTag, e *Product) { n.appendNamedProducts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (ptq *ProductTagQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*ProductTag, init func(*ProductTag), assign func(*ProductTag, *Tenant)) error {
	ids := make([]pulid.ID, 0, len(nodes))
	nodeids := make(map[pulid.ID][]*ProductTag)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (ptq *ProductTagQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*ProductTag, init func(*ProductTag), assign func(*ProductTag, *Product)) error {
	...
	byID := make(map[pulid.ID]*ProductTag)
	nids := make(map[pulid.ID]map[*ProductTag]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(producttag.ProductsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(producttag.ProductsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(producttag.ProductsPrimaryKey[0]))
...
func (ptq *ProductTagQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(producttag.Table, producttag.Columns, sqlgraph.NewFieldSpec(producttag.FieldID, field.TypeString))
	_spec.From = ptq.sql
	...
	if fields := ptq.ctx.Fields; len(fields) > 0 {
		...
		if ptq.withTenant != nil {
			_spec.Node.AddColumnOnce(producttag.FieldTenantID)
		}
	}
...
func (ptq *ProductTagQuery) WithNamedProducts(name string, opts ...func(*ProductQuery)) *ProductTagQuery {
	query := (&ProductClient{config: ptq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if ptq.withNamedProducts == nil {
		ptq.withNamedProducts = make(map[string]*ProductQuery)
	}
	ptq.withNamedProducts[name] = query
	return ptq
}
