...
type ProductQuery struct {
	...
	withNamedProductTags    map[string]*ProductTagQuery
...
func (pq *ProductQuery) QueryProductVariant() *ProductVariantQuery {
	query := (&ProductVariantClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryProductImage() *ProductImageQuery {
...
func (pq *ProductQuery) FirstID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (pq *ProductQuery) FirstIDX(ctx context.Context) pulid.ID {
...
func (pq *ProductQuery) OnlyID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (pq *ProductQuery) OnlyIDX(ctx context.Context) pulid.ID {
...
func (pq *ProductQuery) IDs(ctx context.Context) (ids []pulid.ID, err error) {
...
func (pq *ProductQuery) IDsX(ctx context.Context) []pulid.ID {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withProductVariant: pq.withProductVariant.Clone(),
		withProductImage:   pq.withProductImage.Clone(),
...
func (pq *ProductQuery) WithProductVariant(opts ...func(*ProductVariantQuery)) *ProductQuery {
	query := (&ProductVariantClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProductVariant = query
	return pq
}
...
func (pq *ProductQuery) WithProductImage(opts ...func(*ProductImageQuery)) *ProductQuery {
	query := (&ProductImageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProductImage = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			pq.withProductVariant != nil,
			pq.withProductImage != nil,
		}
	)
	...
	if query := pq.withProductVariant; query != nil {
		if err := pq.loadProductVariant(ctx, query, nodes,
			func(n *Product) { n.Edges.ProductVariant = []*ProductVariant{} },
			func(n *Product, e *ProductVariant) { n.Edges.ProductVariant = append(n.Edges.ProductVariant, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withProductImage; query != nil {
		if err := pq.loadProductImage(ctx, query, nodes,
			func(n *Product) { n.Edges.ProductImage = []*ProductImage{} },
			func(n *Product, e *ProductImage) { n.Edges.ProductImage = append(n.Edges.ProductImage, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedProductTags {
		if err := pq.loadProductTags(ctx, query, nodes,
			func(n *Product) { n.appendNamedProductTags(name) },
			func(n *Product, e *ProductTag) { n.appendNamedProductTags(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedProductVariant {
		if err := pq.loadProductVariant(ctx, query, nodes,
			func(n *Product) { n.appendNamedProductVariant(name) },
			func(n *Product, e *ProductVariant) { n.appendNamedProductVariant(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedProductImage {
		if err := pq.loadProductImage(ctx, query, nodes,
			func(n *Product) { n.appendNamedProductImage(name) },
			func(n *Product, e *ProductImage) { n.appendNamedProductImage(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProductQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Product, init func(*Product), assign func(*Product, *Tenant)) error {
	...
	nodeids := make(map[pulid.ID][]*Product)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (pq *ProductQuery) loadProductTags(ctx context.Context, query *ProductTagQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductTag)) error {
	...
	byID := make(map[pulid.ID]*Product)
	nids := make(map[pulid.ID]map[*Product]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.ProductTagsTable)
		s.Join(joinT).On(s.C(producttag.FieldID), joinT.C(product.ProductTagsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(product.ProductTagsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.ProductTagsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "product_tags" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (pq *ProductQuery) loadProductVariant(ctx context.Context, query *ProductVariantQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductVariant)) error {
	...
	query.Where(predicate.ProductVariant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.ProductVariantColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_product_variant
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_product_variant" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_product_variant" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProductQuery) loadProductImage(ctx context.Context, query *ProductImageQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductImage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[pulid.ID]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ProductImage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.ProductImageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.product_image_product
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_image_product" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_image_product" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *ProductQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withTenant != nil {
			_spec.Node.AddColumnOnce(product.FieldTenantID)
		}
	}
...
// WithNamedProductTags tells the query-builder to eager-load the nodes that are connected to the "product_tags"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithNamedProductTags(name string, opts ...func(*ProductTagQuery)) *ProductQuery {
	query := (&ProductTagClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedProductTags == nil {
		pq.withNamedProductTags = make(map[string]*ProductTagQuery)
	}
	pq.withNamedProductTags[name] = query
	return pq
}

// WithNamedProductVariant tells the query-builder to eager-load the nodes that are connected to the "product_variant"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithNamedProductVariant(name string, opts ...func(*ProductVariantQuery)) *ProductQuery {
	query := (&ProductVariantClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedProductVariant == nil {
		pq.withNamedProductVariant = make(map[string]*ProductVariantQuery)
	}
	pq.withNamedProductVariant[name] = query
	return pq
}

// WithNamedProductImage tells the query-builder to eager-load the nodes that are connected to the "product_image"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithNamedProductImage(name string, opts ...func(*ProductImageQuery)) *ProductQuery {
	query := (&ProductImageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedProductImage == nil {
		pq.withNamedProductImage = make(map[string]*ProductImageQuery)
	}
	pq.withNamedProductImage[name] = query
	return pq
}
