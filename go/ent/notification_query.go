...
func (nq *NotificationQuery) QueryTenant() *TenantQuery {
...
func (nq *NotificationQuery) QueryConnection() *ConnectionQuery {
	query := (&ConnectionClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) QueryEmailTemplate() *EmailTemplateQuery {
	query := (&EmailTemplateClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NotificationQuery) FirstID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (nq *NotificationQuery) FirstIDX(ctx context.Context) pulid.ID {
...
func (nq *NotificationQuery) OnlyID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (nq *NotificationQuery) OnlyIDX(ctx context.Context) pulid.ID {
...
func (nq *NotificationQuery) IDs(ctx context.Context) (ids []pulid.ID, err error) {
...
func (nq *NotificationQuery) IDsX(ctx context.Context) []pulid.ID {
	ids, err := nq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withConnection:    nq.withConnection.Clone(),
		withEmailTemplate: nq.withEmailTemplate.Clone(),
...
func (nq *NotificationQuery) WithTenant(opts ...func(*TenantQuery)) *NotificationQuery {
	query := (&TenantClient{config: nq.config}).Query()
...
func (nq *NotificationQuery) WithConnection(opts ...func(*ConnectionQuery)) *NotificationQuery {
	query := (&ConnectionClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withConnection = query
	return nq
}
...
func (nq *NotificationQuery) WithEmailTemplate(opts ...func(*EmailTemplateQuery)) *NotificationQuery {
	query := (&EmailTemplateClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withEmailTemplate = query
	return nq
}
...
func (nq *NotificationQuery) prepareQuery(ctx context.Context) error {
	...
	if notification.Policy == nil {
		return errors.New("ent: uninitialized notification.Policy (forgotten import ent/runtime?)")
	}
	if err := notification.Policy.EvalQuery(ctx, nq); err != nil {
		return err
	}
	return nil
}
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	var (
		...
		loadedTypes = [3]bool{
			nq.withTenant != nil,
			nq.withConnection != nil,
			nq.withEmailTemplate != nil,
		}
	)
	if nq.withConnection != nil || nq.withEmailTemplate != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withConnection; query != nil {
		if err := nq.loadConnection(ctx, query, nodes, nil,
			func(n *Notification, e *Connection) { n.Edges.Connection = e }); err != nil {
			return nil, err
		}
	}
	if query := nq.withEmailTemplate; query != nil {
		if err := nq.loadEmailTemplate(ctx, query, nodes, nil,
			func(n *Notification, e *EmailTemplate) { n.Edges.EmailTemplate = e }); err != nil {
			return nil, err
		}
	}
...
func (nq *NotificationQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Tenant)) error {
	ids := make([]pulid.ID, 0, len(nodes))
	nodeids := make(map[pulid.ID][]*Notification)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (nq *NotificationQuery) loadConnection(ctx context.Context, query *ConnectionQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Connection)) error {
	ids := make([]pulid.ID, 0, len(nodes))
	nodeids := make(map[pulid.ID][]*Notification)
	for i := range nodes {
		if nodes[i].notification_connection == nil {
			continue
		}
		fk := *nodes[i].notification_connection
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_connection" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NotificationQuery) loadEmailTemplate(ctx context.Context, query *EmailTemplateQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *EmailTemplate)) error {
	ids := make([]pulid.ID, 0, len(nodes))
	nodeids := make(map[pulid.ID][]*Notification)
	for i := range nodes {
		if nodes[i].notification_email_template == nil {
			continue
		}
		fk := *nodes[i].notification_email_template
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "notification_email_template" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
