...
func (atq *APITokenQuery) QueryTenant() *TenantQuery {
	query := (&TenantClient{config: atq.config}).Query()
...
func (atq *APITokenQuery) FirstID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (atq *APITokenQuery) FirstIDX(ctx context.Context) pulid.ID {
...
func (atq *APITokenQuery) OnlyID(ctx context.Context) (id pulid.ID, err error) {
	var ids []pulid.ID
...
func (atq *APITokenQuery) OnlyIDX(ctx context.Context) pulid.ID {
...
func (atq *APITokenQuery) IDs(ctx context.Context) (ids []pulid.ID, err error) {
...
func (atq *APITokenQuery) IDsX(ctx context.Context) []pulid.ID {
	ids, err := atq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (atq *APITokenQuery) Clone() *APITokenQuery {
	...
	return &APITokenQuery{
		...
		withTenant: atq.withTenant.Clone(),
...
func (atq *APITokenQuery) WithTenant(opts ...func(*TenantQuery)) *APITokenQuery {
	query := (&TenantClient{config: atq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	atq.withTenant = query
	return atq
}
...
//		GroupBy(apitoken.FieldTenantID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (atq *APITokenQuery) prepareQuery(ctx context.Context) error {
	...
	if apitoken.Policy == nil {
		return errors.New("ent: uninitialized apitoken.Policy (forgotten import ent/runtime?)")
	}
...
func (atq *APITokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIToken, error) {
	...
	if query := atq.withTenant; query != nil {
		if err := atq.loadTenant(ctx, query, nodes, nil,
			func(n *APIToken, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
...
func (atq *APITokenQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*APIToken, init func(*APIToken), assign func(*APIToken, *Tenant)) error {
	ids := make([]pulid.ID, 0, len(nodes))
	nodeids := make(map[pulid.ID][]*APIToken)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (atq *APITokenQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*APIToken, init func(*APIToken), assign func(*APIToken, *User)) error {
	...
	nodeids := make(map[pulid.ID][]*APIToken)
	for i := range nodes {
		if nodes[i].user_api_token == nil {
			continue
		}
		fk := *nodes[i].user_api_token
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_api_token" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (atq *APITokenQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := atq.ctx.Fields; len(fields) > 0 {
		...
		if atq.withTenant != nil {
			_spec.Node.AddColumnOnce(apitoken.FieldTenantID)
		}
	}
